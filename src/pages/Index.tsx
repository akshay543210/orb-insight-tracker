...
const Index = () => {
  ...
  return <ErrorBoundary>
      <FloatingParticles />
      <motion.div className="space-y-6 relative z-10" initial={{
      opacity: 0,
      y: 20
    }} animate={{
      opacity: 1,
      ...
      duration: 0.4
        ...
        delay: 0.1
          ...
          <div>
            <motion.h1 className="text-4xl font-bold font-cyber text-glow-primary mb-2" animate={{
            textShadow: ["var(--text-glow-primary)", "var(--text-glow-accent)", "var(--text-glow-primary)"]
          }} transition={{
            duration: 3,
            repeat: Infinity
          }}>
            ...
            <p className="text-muted-foreground font-tech text-lg">Trading Dashboard</p>
          ...
          <motion.div className="text-right bg-gradient-card p-4 rounded-lg border border-primary/30 shadow-primary/20" whileHover={{
          scale: 1.05,
          boxShadow: "var(--shadow-primary)"
            ...
            {activeAccount && <div className={`text-sm font-medium font-tech ${activeAccount.current_balance >= activeAccount.starting_balance ? 'text-success text-glow-accent' : 'text-destructive text-glow-pink'}`}>
        ...
        {/* Quick Stats */}
        <motion.div className="grid grid-cols-2 md:grid-cols-6 gap-4" initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
        ...
        staggerChildren: 0.1
          ...
          <StatsCard title="WINS" value={stats.wins.toString()} change={`${stats.winRate.toFixed(0)}%`} positive={true} icon={<TrendingUp className="w-5 h-5" />} />
          <StatsCard title="LOSSES" value={stats.losses.toString()} change={`${(100 - stats.winRate).toFixed(0)}%`} positive={false} icon={<TrendingDown className="w-5 h-5" />} />
          <StatsCard title="BREAKEVEN" value={stats.breakevens.toString()} change={`${stats.totalTrades > 0 ? (stats.breakevens / stats.totalTrades * 100).toFixed(0) : 0}%`} icon={<Target className="w-5 h-5" />} />
          ...
          <StatsCard title="TOTAL" value={`${stats.totalRR > 0 ? '+' : ''}${stats.totalRR.toFixed(1)}R`} positive={stats.totalRR >= 0} icon={<TrendingUp className="w-5 h-5" />} />
        </motion.div>
        ...
        scale: 1.02
          ...
          <Card className="bg-gradient-card shadow-primary/20 border-primary/30 hover:shadow-primary/40 transition-all duration-500">
            ...
            <CardContent>
              <ErrorBoundary fallback={<div className="h-64 flex items-center justify-center text-muted-foreground">
                  ...
                  <div className="h-64 relative">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={equityCurveData}>
                        ...
                        <XAxis dataKey="date" stroke="hsl(var(--primary))" fontSize={12} fontFamily="Rajdhani" />
                        <YAxis stroke="hsl(var(--primary))" fontSize={12} fontFamily="Rajdhani" domain={stats.totalTrades > 0 ? ['dataMin - 1', 'dataMax + 1'] : [0, 1]} label={{
                        value: 'R/R',
                        angle: -90,
                        position: 'insideLeft'
                      }} />
